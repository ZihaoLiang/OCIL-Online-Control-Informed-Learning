...
class CartPole:
    ...
    def initCost(self, wx=None, wq=None, wdx=None, wdq=None, wu=0.001):
        ...
        if wdq is None:
            ...
            self.wdq = wdq
        self.cost_auxvar = vcat(parameter)
        self.n_auxvar = self.cost_auxvar.numel()
    ...
    # def init_step_neural_obj(self, hidden_layers=None):
    #     if hidden_layers is None:
    #         hidden_layers=[self.n_state]
    #     self.setNeuralCost(hidden_layers)
         
    def initNeuralCost(self, hidden_layers, wu=0.0001):
        # Use neural network to represent the objective.
        # Note that here we use auxvar to denote the parameter of the neural objective
        ...
        u = self.U
        cost_auxvar = []
        Ak = SX.sym('Ak', layers[0], self.X.size()[0])  # weights matrix
        bk = SX.sym('bk', layers[0])  # bias vector
        # cost_auxvar = vertcat(cost_auxvar, Ak.reshape((-1, 1)))
        # cost_auxvar = vertcat(cost_auxvar, bk)
        ...
        # self.cost_auxvar = cost_auxvar
        # self.n_auxvar = self.cost_auxvar.numel()
        self.n_auxvar = self.cost_auxvar.shape[0]
        # self.path_cost = a * a + wu * u * u
        # self.final_cost = a * a
        self.path_cost = norm_2(a)**2 + wu * norm_2(u)**2
        self.final_cost = norm_2(a)**2
