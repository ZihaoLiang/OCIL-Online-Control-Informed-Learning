...
############################### PointMass system ###############################
class Dot:
    def _init_(self, project_name = 'Dot'):
    ...
    def Dyn(self, nT):
        ...
        for i in range(nT):
            x1 = self.X[0,i]
            x2 = self.X[1,i]
            self.f.append(vertcat(self.U[0,i], self.U[1,i]))
    ...
    def Cost(self, w, target, nT):
        ...
        for i in range(nT):
            ...
            # cost for u1
            self.cost_u1 = dot(self.u1[i], self.u1[i])
            # cost for u2
            self.cost_u2 = dot(self.u2[i], self.u2[i])
            
            self.phi = np.array([self.cost_x1, self.cost_x2,self.cost_u1,self.cost_u2])  #feature
