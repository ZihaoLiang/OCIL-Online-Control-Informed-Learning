...
class Pend:
    ...
    def Dyn(self, nT):
        ...
        for i in range(nT):
            ...
            self.f.append(vertcat(x2, (self.U[i] - m * g * l * sin(x1) - d * x2) / I))
...
############################### CartPole ###############################
class CartPole:
    def _init_(self, project_name = "CartPole"):
    ...
    def Dyn(self, nT):
        ...
        for i in range(nT):
            ...
            ddq = (-u * cos(q) - mp * l * dq * dq * sin(q) * cos(q) - (mc + mp) * g * sin(q)) / (
                        l * mc + l * mp * sin(q) * sin(q))  # acceleration of theta
            
            self.f.append(vertcat(dx, dq, ddx, ddq))
    ...
    def Cost(self, w, target, nT):
        ...
        for i in range(nT):
            ...
            # state cost
            self.cost_x = (x - target[0]) ** 2 

            self.cost_q = (q - target[1]) ** 2

            self.cost_dx = (dx - target[2]) ** 2

            self.cost_dq = (dq - target[3]) ** 2
            
            # input cost
            self.cost_u = u * u

            self.phi = np.array([self.cost_x, self.cost_q, self.cost_dx, self.cost_dq,self.cost_u])  #feature
...
class UAV:
    ...
    def Cost(self, w, target, nT):
        ...
        for i in range(nT):
            ...
            self.phi = np.array([self.cost_p_I, self.cost_v_I, self.cost_q, self.cost_w_B, self.cost_thrust])   #feature
...
############################### Rocket ###############################
class Rocket:
    def _init_(self, project_name = "Rocket"):
        self.project_name = project_name
    ...
    def Dyn(self, nT):
        ...
        self.opti = casadi.Opti()
        self.X = self.opti.variable(13, nT + 1)
        self.U = self.opti.variable(3, nT) #input
        ...
        J_B = diag(vertcat(Jx, Jy, Jz))
        # Gravity
        g_I = vertcat(-g, 0, 0)
        # Vector from thrust point to CoM
        r_T_B = vertcat(-l / 2, 0, 0)
        ...
        for i in range(nT):
            ...
            dr_I = v_I
            dv_I = 1 / m * mtimes(C_I_B, T_B) + g_I

            dq = 1 / 2 * mtimes(self.omega(w_B), q)
            dw = mtimes(inv(J_B),
                        mtimes(self.skew(r_T_B), T_B) -
                        mtimes(mtimes(self.skew(w_B), J_B), w_B))


            self.f.append(vertcat(dr_I, dv_I, dq, dw))
    ...
    def Cost(self, w, target, nT):
        ...
        for i in range(nT):
            ...
            # goal position in the world frame
            goal_r_I = target[0:3]
            self.cost_r_I = dot(r_I - goal_r_I, r_I - goal_r_I)

            # goal velocity
            goal_v_I = target[3:6]
            self.cost_v_I = dot(v_I - goal_v_I, v_I - goal_v_I)

            # tilt angle upward direction of rocket should be close to upward of earth
            C_I_B = transpose(self.dir_cosine(q))
            nx = np.array([1., 0., 0.])
            ny = np.array([0., 1., 0.])
            nz = np.array([0., 0., 1.])
            ...
            self.cost_side_thrust = (T_B[1] ** 2 + T_B[2] ** 2)

            # the thrust cost
            self.cost_thrust = dot(T_B, T_B)

            # auglar velocity cost
            goal_w_B = target[10:13]
            self.cost_w_B = dot(w_B - goal_w_B, w_B - goal_w_B)

            self.phi = np.array([self.cost_r_I, self.cost_v_I, self.cost_w_B, self.cost_tilt, self.cost_side_thrust, self.cost_thrust])  #feature
